  ...
  return (
    <div className="p-4 max-w-4xl mx-auto space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="border rounded p-3 space-y-3">
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide">Neo4j</p>
            <h2 className="font-semibold text-lg">Recomendaciones</h2>
            <p className="text-sm text-gray-500">GET /recommend/:user_id</p>
          </div>
          <div className="flex gap-2">
            <input
              className="flex-1 border rounded px-2 py-1"
            ...
            <button
              className="px-3 py-1 border rounded"
              onClick={() => recUserId && goTo(`/recommend/${recUserId}`)}
            >
              Abrir
            </button>
          </div>
          <p className="text-xs text-gray-500">
            Para que funcione, antes cargá progreso desde{" "}
            <Link to="/recommend/progress" className="underline">
              este formulario
            </Link>.
          </p>
        </div>
        ...
        <div className="border rounded p-3 space-y-3">
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide">Neo4j</p>
            <h2 className="font-semibold text-lg">Registrar progreso</h2>
            <p className="text-sm text-gray-500">POST /recommend/progress</p>
          </div>
          <form className="space-y-2" onSubmit={handleProgress}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <input
                className="border rounded px-2 py-1 w-full"
                placeholder="user_id"
                value={progUser}
                onChange={(e) => setProgUser(e.target.value)}
              />
              <input
                className="border rounded px-2 py-1 w-full"
                placeholder="course_id"
                value={progCourse}
                onChange={(e) => setProgCourse(e.target.value)}
              />
            </div>
            <input
              className="border rounded px-2 py-1 w-full"
            ...
            <div className="flex items-center gap-2">
              ...
              {progStatus && <span className="text-xs text-gray-500">{progStatus}</span>}
            </div>
          </form>
        </div>
      </section>
